      ...
      try {
        ...
        const renderAttainmentParent = (rows: Array<{studentId:string;name:string;value:number}>, label: string) => {
          ...
          result.innerHTML =
            ...
            '<section class="card"><h2>3. Student Groups Requiring Action</h2><table><thead><tr><th>Group</th><th>Students</th><th>Doable classroom action</th><th>Extra / outside-class action</th><th>How to measure</th></tr></thead><tbody>' +
            '<tr><td><span class="pill r">Below</span></td><td>' + names(priority) + '</td><td>Teacher-led 10-minute focus group 2× weekly; reteach one gap at a time; model one example, complete one together, then one independently; pair support; finish with a 3-question exit check.</td><td>10–15 minute targeted practice twice weekly using the same weak skill; short vocabulary/reading/spelling/writing task as appropriate; optional lunchtime/after-school catch-up for students who remain below target.</td><td>Record exit check /5 or mini-check %. Compare a weekly score with the baseline. Target: reach ≥50% first, then sustain it on two checks.</td></tr>' +
            '<tr><td><span class="pill y">In line</span></td><td>' + names(inlineRows) + '</td><td>Short consolidation task followed by one challenge/application question; targeted feedback on one improvement point; purposeful pair discussion before independent response.</td><td>One short challenge task weekly: reading response, vocabulary application, corrected writing paragraph, listening note-taking or speaking recording according to the assessed skill.</td><td>Weekly 5-item check or rubric score. Target: maintain ≥50% and move toward ≥70% on two consecutive checks.</td></tr>' +
            '<tr><td><span class="pill g">Above</span></td><td>' + names(aboveRows) + '</td><td>Extension question, deeper reasoning/application, independent task and occasional peer-explanation role without replacing the teacher.</td><td>Optional extension: independent reading/research, advanced response, presentation, vocabulary-in-context task or extended writing linked to the current unit.</td><td>Challenge rubric / mini-assessment. Target: sustain ≥70% while completing higher-level work accurately and independently.</td></tr></tbody></table></section>' +
            '<section class="card"><h2>4. SMART Improvement Target</h2><p class="analysis"><b>Target:</b> ' + targetText + '</p><p><b>Success measure:</b> use the same short measure consistently (for example /5 exit check, /10 mini-quiz, skill rubric or comparable assessment). Record baseline → target → review result so improvement is visible rather than based on impression.</p></section>' +
            '<section class="card"><h2>5. Teaching & Intervention Action Plan</h2><table><thead><tr><th>Need / group</th><th>Action</th><th>Frequency</th><th>Measure</th><th>Target</th><th>Review & next decision</th></tr></thead><tbody>' +
            '<tr><td>Priority intervention<br><b>' + belowRows.length + ' students</b></td><td>10-minute small-group reteach → guided example → independent attempt → immediate correction. Use the exact weak skill from the selected evidence.</td><td>2× weekly for 3 weeks</td><td>3–5 question exit check after intervention + weekly mini-check.</td><td>Each student reaches ≥50% and shows improvement from baseline on two consecutive checks.</td><td>After 3 weeks: if achieved, move to consolidation; if not, narrow the skill gap further and intensify support.</td></tr>' +
            '<tr><td>Consolidation / acceleration<br><b>' + inlineRows.length + ' students</b></td><td>Brief retrieval practice, targeted feedback, application task and one above-level challenge item.</td><td>Weekly within normal lessons</td><td>5-item check, marked class task or skill rubric.</td><td>Maintain curriculum standard and move toward ≥70%.</td><td>Review at next comparable assessment; move successful students to extension.</td></tr>' +
            '<tr><td>Extension<br><b>' + aboveRows.length + ' students</b></td><td>Higher-order application, independent response, deeper questioning and extension linked to the same curriculum objective.</td><td>At least 1 extension opportunity weekly</td><td>Extension rubric/task + next comparable assessment.</td><td>Sustain ≥70% with evidence of independent higher-level application.</td><td>Continue challenge; increase complexity if performance remains secure.</td></tr></tbody></table></section>' +
            '<section class="card"><h2>6. Quick Intervention Activities</h2><p><b>During the lesson:</b> 5-minute retrieval starter; mini whiteboard check; teacher focus table while others work independently; model–together–alone practice; paired correction; 3-question exit ticket.</p><p><b>Friday Vocabulary Bridge Detectives:</b> Students work in groups on the department Vocabulary Bridge Detectives project, consolidating the weekly English, Math and Science vocabulary through meaning, context and visual/drawing tasks. <b>At the same time, the teacher sits with the identified weak students for a focused small-group intervention</b> using their actual gaps from the analysis. Reteach one skill at a time, model it, practise together, then check each student independently before returning them to the project.</p><p><b>Friday measurement:</b> use a very short 3–5 question skill check with the intervention group. Record the result each Friday as <b>Baseline → Friday Check 1 → Friday Check 2 → Friday Check 3 → Review Assessment</b>. This makes the weekly intervention visible and measurable.</p><p><b>5–10 minute intervention:</b> vocabulary picture/definition sort; sentence correction; short paragraph with 3 comprehension questions; spelling look-cover-write-check; listening replay with key-word notes; speaking prompt with a simple success checklist.</p><p><b>Outside class:</b> one short targeted practice twice weekly, corrected and returned; optional lunch/after-school catch-up; student correction task using errors from the previous assessment; short reading or writing application linked to the current unit.</p><p><b>Keep evidence simple:</b> record <b>Baseline → Check 1 → Check 2 → Review assessment</b>. Use the same scale or convert each result to a percentage. This provides visible evidence of response to intervention.</p></section>' +
