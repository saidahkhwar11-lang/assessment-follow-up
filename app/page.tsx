      ...
      try {
        ...
        const renderAttainmentParent = (rows: Array<{studentId:string;name:string;value:number}>, label: string) => {
          ...
          result.innerHTML =
            ...
            (() => {
              const specificSkill = label.toLowerCase().includes("diagnostic ·") || (!["Diagnostic total","CA total marks"].includes(label));
              const rowsMap = [
                '<tr><td><b>1.1.1</b></td><td>Attainment against authorised/licensed curriculum standards</td><td><b>Evidence available</b> — ' + safe(label) + ' provides the selected attainment evidence for this class.</td></tr>',
                specificSkill ? '<tr><td><b>1.1.3</b></td><td>Knowledge, skills and understanding</td><td><b>Evidence available</b> — this selected skill/assessment identifies strengths and gaps in students’ knowledge, skills and understanding.</td></tr>' : '',
                '<tr><td><b>3.2.3</b></td><td>Analysis of assessment data to monitor students’ progress</td><td><b>Supporting evidence</b> — class and individual results are analysed and priority students are identified.</td></tr>',
                '<tr><td><b>3.2.4</b></td><td>Use of assessment information to influence teaching/curriculum</td><td><b>Supporting evidence</b> — identified needs are converted into specific teaching and intervention actions.</td></tr>',
                '<tr><td><b>3.2.5</b></td><td>Teachers’ knowledge of students and support/challenge</td><td><b>Supporting evidence</b> — the plan identifies students for differentiated support and challenge, to be evidenced through implementation and follow-up.</td></tr>'
              ].filter(Boolean).join("");
              return '<section class="card"><h2>2. Framework Evidence Mapping</h2><p><b>Selected analysis:</b> ' + safe(label) + '. Only framework indicators directly relevant to this selected evidence are shown.</p><table><thead><tr><th>Indicator</th><th>Framework focus</th><th>How this analysis provides evidence</th></tr></thead><tbody>' + rowsMap + '</tbody></table></section>';
            })() +
