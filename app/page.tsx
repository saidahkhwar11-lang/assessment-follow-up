      ...
      try {
        ...
        const renderAttainmentParent = (rows: Array<{studentId:string;name:string;value:number}>, label: string) => {
          ...
          result.innerHTML =
            ...
            '<section class="card"><h2>3. Student Groups Requiring Action</h2><table><thead><tr><th>Group</th><th>Students</th><th>Teaching response</th></tr></thead><tbody><tr><td><span class="pill g">Above</span></td><td>' + names(aboveRows) + '</td><td>Extension, higher-order questioning and independent application.</td></tr><tr><td><span class="pill y">In line</span></td><td>' + names(inlineRows) + '</td><td>Consolidation, targeted feedback and challenge toward above expectations.</td></tr><tr><td><span class="pill r">Below</span></td><td>' + names(priority) + '</td><td>Focused reteaching, scaffolded practice, frequent formative checks and follow-up.</td></tr></tbody></table></section>' +
            '<section class="card"><h2>4. SMART Improvement Target</h2><p class="analysis"><b>Target:</b> ' + targetText + '</p><p><b>Success measure:</b> the next comparable assessment will be used to check the percentage at/above expectations and individual movement from each student’s starting point.</p></section>' +
            '<section class="card"><h2>5. Teaching & Intervention Action Plan</h2><table><thead><tr><th>Priority</th><th>Action</th><th>Students</th><th>Success criteria</th><th>Evidence / Review</th></tr></thead><tbody><tr><td>Below expectations</td><td>Reteach identified knowledge/skills in small steps; model responses; guided practice; immediate feedback.</td><td>' + (belowRows.length ? belowRows.length + ' students' : 'As identified') + '</td><td>Students demonstrate secure performance at ≥50% and close identified gaps.</td><td>Exit tickets, classwork and next comparable assessment.</td></tr><tr><td>In line</td><td>Consolidate core skills and add graduated challenge, application and independent practice.</td><td>' + inlineRows.length + ' students</td><td>Movement toward ≥70% while maintaining secure curriculum-standard performance.</td><td>Formative checks and next assessment.</td></tr><tr><td>Above expectations</td><td>Provide extension, deeper application, reasoning and independent tasks.</td><td>' + aboveRows.length + ' students</td><td>Maintain ≥70% and demonstrate sustained challenge.</td><td>Extended tasks and next assessment.</td></tr></tbody></table></section>' +
            '<section class="card"><h2>6. Monitoring & Next Assessment</h2><p>Use ongoing formative assessment to check response to teaching. Reassess using a <b>comparable curriculum-aligned evidence point</b>. When the later result is available, use <b>Progress Analysis</b> to compare matched students against their starting points and review whether the intervention should continue, intensify or move to greater challenge.</p></section>' +
            '<section class="card"><h2>7. Student Attainment Profile</h2><table><thead><tr><th>Student ID</th><th>Student name</th><th>Result</th><th>Level</th><th>Next step</th></tr></thead><tbody>' + rows.map(x => { const lev = x.value >= 70 ? ["Above expectations","g","Extend and challenge"] : x.value >= 50 ? ["In line with expectations","y","Consolidate and move toward above"] : ["Below expectations","r","Targeted intervention and follow-up"]; return '<tr><td>' + safe(x.studentId) + '</td><td><b>' + safe(x.name) + '</b></td><td>' + x.value + '%</td><td><span class="pill ' + lev[1] + '">' + lev[0] + '</span></td><td>' + lev[2] + '</td></tr>'; }).join("") + '</tbody></table></section>';
        };
