      ...
      try {
        ...
        const renderAttainmentParent = (rows: Array<{studentId:string;name:string;value:number}>, label: string) => {
          ...
          result.innerHTML =
            ...
            '<section class="card"><h2>2. Strengths & Areas for Improvement</h2><p><b>Strength:</b> ' + (aboveRows.length ? proportion(ap) + ' of students are currently above expectations in ' + safe(label) + '.' : 'No students are currently above expectations in this evidence set.') + '</p><p><b>Area for improvement:</b> ' + (belowRows.length ? proportion(bp) + ' of students are below expectations and require focused support before the next comparable assessment.' : 'No students are below expectations in this evidence set; teaching should focus on maintaining attainment and increasing challenge.') + '</p></section>' +
            (() => {
              ...
              return '<section class="card"><h2>3. Recommended Teaching Strategies</h2><p><b>Selected for this need:</b> ' + recommendedStrategies.map(x => safe(x)).join(' · ') + '</p><p>These strategies are adapted to the identified skill need from BridgeUniverse guidance for supporting struggling English language learners; they complement the UAE School Inspection Framework-based analysis.</p></section><section class="card"><h2>4. Student Groups Requiring Action</h2><p><b>Plan generated for:</b> ' + safe(skill) + ' — ' + safe(p.focus) + '.</p><table><thead><tr><th>Group</th><th>Students</th><th>Specific response</th><th>Measurement</th></tr></thead><tbody>' +
                ...
                '<section class="card"><h2>5. SMART Improvement Target</h2><p class="analysis"><b>Class target:</b> ' + targetText + '</p><p><b>' + safe(skill) + ' intervention target:</b> ' + safe(p.target) + '</p></section>' +
                '<section class="card"><h2>6. Teaching & Intervention Action Plan — ' + safe(skill) + '</h2><table><thead><tr><th>When</th><th>Action</th><th>Who</th><th>Measure</th><th>Decision</th></tr></thead><tbody>' +
