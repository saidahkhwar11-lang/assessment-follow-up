    ...
    win.addEventListener("load", () => {
      try {
        ...
        const renderAttainmentParent = (rows: Array<{studentId:string;name:string;value:number}>, label: string) => {
          ...
          const avg = Math.round(rows.reduce((sum, x) => sum + x.value, 0) / rows.length);
          const aboveRows = rows.filter(x => x.value >= 70);
          const inlineRows = rows.filter(x => x.value >= 50 && x.value < 70);
          const belowRows = rows.filter(x => x.value < 50);
          const priority = belowRows.slice().sort((a,b) => a.value - b.value);
          const targetAt = Math.max(75, atp);
          const targetAbove = Math.max(50, ap);
          const targetText = atp < 75
            ? 'Increase students attaining at or above curriculum expectations from ' + atp + '% to at least 75% by the next comparable assessment cycle.'
            : 'Maintain at least ' + atp + '% at or above curriculum expectations and increase the proportion above expectations from ' + ap + '% toward at least ' + targetAbove + '%.';
          const names = (items: typeof rows) => items.length ? items.map(x => safe(x.name) + ' (' + x.value + '%)').join(', ') : 'None identified from this evidence.';
          result.innerHTML =
            '<div class="notice"><b>Attainment evidence:</b> ' + safe(label) + '. This is a baseline/attainment judgement from one evidence point; it is not a progress judgement.</div>' +
            '<div class="stats"><div class="stat"><span>Students analysed</span><b>' + rows.length + '</b></div><div class="stat"><span>Class average</span><b>' + avg + '%</b></div><div class="stat"><span>Above expectations</span><b>' + ap + '%</b></div><div class="stat"><span>At or above</span><b>' + atp + '%</b></div><div class="stat"><span>Judgement</span><b>' + j + '</b></div></div>' +
            '<section class="card"><h2>1. Framework Attainment Analysis</h2><div class="bar"><i class="g" style="width:' + ap + '%"></i><i class="y" style="width:' + ip + '%"></i><i class="r" style="width:' + bp + '%"></i></div><p><b>' + ap + '%</b> Above (70%+) · <b>' + ip + '%</b> In line (50–69%) · <b>' + bp + '%</b> Below (&lt;50%)</p><p class="analysis"><b>' + comment(j) + '</b> Judgement: <b>' + j + '</b>. At or above expectations: <b>' + atp + '%</b>. Framework proportion used: <b>' + proportion(j === "Acceptable" ? atp : ap) + '</b>.</p></section>' +
            '<section class="card"><h2>2. Framework Evidence Mapping</h2><table><thead><tr><th>Indicator</th><th>Framework focus</th><th>Status for this analysis</th></tr></thead><tbody>' +
            '<tr><td><b>1.1.1</b></td><td>Attainment against authorised/licensed curriculum standards</td><td><b>Evidence available</b> — the selected attainment evidence is used for the current judgement.</td></tr>' +
            '<tr><td><b>1.1.2</b></td><td>Attainment against national/international standards</td><td>Not claimed unless the selected assessment is explicitly benchmarked to an appropriate national/international standard.</td></tr>' +
            '<tr><td><b>1.1.3</b></td><td>Knowledge, skills and understanding</td><td><b>Supporting evidence</b> — especially when a specific skill/assessment is selected.</td></tr>' +
            '<tr><td><b>1.1.4</b></td><td>Trends over time</td><td>Not yet evidenced by one assessment point; requires suitable evidence over time.</td></tr>' +
            '<tr><td><b>1.2.1</b></td><td>Progress against starting points and over time</td><td>Use <b>Progress Analysis</b> when two valid comparable evidence points are available.</td></tr>' +
            '<tr><td><b>1.2.2</b></td><td>Progress in lessons</td><td>Not judged from assessment marks alone; support with lesson/formative evidence.</td></tr>' +
            '<tr><td><b>1.2.3</b></td><td>Progress of different groups</td><td>Not fully judged unless reliable group information and comparable progress evidence are available.</td></tr>' +
            '<tr><td><b>3.2.3</b></td><td>Analysis of assessment data to monitor students’ progress</td><td><b>Supporting evidence</b> — class, group and individual results are analysed and priority students identified.</td></tr>' +
            '<tr><td><b>3.2.4</b></td><td>Use of assessment information to influence teaching/curriculum</td><td><b>Supporting evidence</b> — the generated intervention plan links identified needs to teaching actions.</td></tr>' +
            '<tr><td><b>3.2.5</b></td><td>Teachers’ knowledge of students and support/challenge</td><td><b>Supporting evidence</b> when the teacher implements, monitors and records the differentiated support/challenge shown in this plan.</td></tr>' +
            '</tbody></table><p><small><b>Integrity note:</b> The tracker maps only evidence supported by the selected data. It does not automatically claim indicators that require additional evidence.</small></p></section>' +
            '<section class="card"><h2>3. Strengths & Areas for Improvement</h2><p><b>Strength:</b> ' + (aboveRows.length ? proportion(ap) + ' of students are currently above expectations in ' + safe(label) + '.' : 'No students are currently above expectations in this evidence set.') + '</p><p><b>Area for improvement:</b> ' + (belowRows.length ? proportion(bp) + ' of students are below expectations and require focused support before the next comparable assessment.' : 'No students are below expectations in this evidence set; teaching should focus on maintaining attainment and increasing challenge.') + '</p></section>' +
            (() => {
              const evidence = label.toLowerCase();
              const skill =
                evidence.includes("vocab") ? "Vocabulary" :
                evidence.includes("reading") ? "Reading" :
                evidence.includes("grammar") ? "Grammar" :
                evidence.includes("spelling") ? "Spelling" :
                evidence.includes("writing") ? "Writing" :
                evidence.includes("speaking") ? "Speaking" :
                evidence.includes("listening") ? "Listening" :
                evidence.includes("context") ? "Context Clues" : "General English";
              const plans: Record<string,{focus:string;classroom:string;friday:string;outside:string;measure:string;target:string;extension:string}> = {
                "Vocabulary": {focus:"word meaning, retrieval and accurate use in context",classroom:"Use picture/definition matching, Frayer-style word cards, word families, sentence completion and short context application. Recycle the weakest words through retrieval rather than copying.",friday:"Use Vocabulary Bridge Detectives for the whole class. While groups investigate the weekly English, Math and Science words, teach the priority group the vocabulary they missed: meaning → example → student sentence → quick retrieval.",outside:"Assign a short online vocabulary worksheet in the LMS plus an extra paper worksheet for targeted practice. Students use each target word in a meaningful sentence/context. Teacher remains available to support priority students at suitable times during the school day.",measure:"5-word retrieval/context check: meaning + correct use. Record % correct.",target:"Reach ≥80% accuracy on two consecutive vocabulary checks.",extension:"Use target vocabulary in unfamiliar contexts, word-family work and student-created examples."},
                "Reading": {focus:"comprehension, evidence finding and inference",classroom:"Use one short text. Model think-aloud reading, underline key evidence, teach how to locate the answer, then move from literal questions to inference. Students must point to the sentence that supports each answer.",friday:"While the class completes Vocabulary Bridge Detectives, read a short differentiated passage with the priority group. Teacher models one question, group answers one together, then students answer 3 independently with evidence.",outside:"Assign a short reading worksheet in the LMS plus an extra paper comprehension worksheet. Students correct wrong answers using evidence from the text. Teacher provides additional support to priority students whenever suitable during the school day.",measure:"Reading mini-check: percentage of comprehension questions correct, with separate note for literal/inference errors.",target:"Reach ≥70% comprehension accuracy on two comparable short texts.",extension:"Longer/unfamiliar text, inference, author purpose and evidence-based justification."},
                "Grammar": {focus:"accurate grammar in sentences and application",classroom:"Use errors from students’ work. Model the rule briefly, correct examples together, then students edit and create their own sentences. Avoid isolated rule copying.",friday:"During Vocabulary Bridge Detectives, take the priority group for a sentence clinic: identify the error → explain the rule → correct together → write one accurate independent sentence.",outside:"Assign a short grammar worksheet in the LMS plus an extra paper correction/application worksheet. Teacher provides additional support to priority students whenever suitable during the school day.",measure:"5-sentence grammar application check; record number correct and recurring error type.",target:"Achieve at least 4/5 correct on two consecutive application checks.",extension:"Apply the same grammar accurately in paragraph writing and sentence transformation."},
                "Spelling": {focus:"spelling patterns, retrieval and transfer to writing",classroom:"Group misspelled words by pattern; use look-cover-write-check, word building, mini-dictation and immediate correction. Recheck the same words later rather than once only.",friday:"During Vocabulary Bridge Detectives, run a 10-minute spelling clinic with the priority group using their actual error words and patterns, ending with a short dictation.",outside:"Assign an online spelling worksheet in the LMS plus an extra paper worksheet based on each student’s error patterns. Teacher provides additional support to priority students whenever suitable during the school day.",measure:"Weekly 10-word dictation plus transfer check in a sentence.",target:"Reach ≥80% spelling accuracy on two consecutive checks and spell target words correctly in context.",extension:"Word origins, affixes, advanced patterns and accurate use in extended writing."},
                "Writing": {focus:"planning, organisation, sentence quality and editing",classroom:"Use a short model and success criteria. Teacher models one paragraph, students plan with key points, draft a short section, then improve one criterion at a time through feedback and editing.",friday:"During Vocabulary Bridge Detectives, conference with the priority group. Work on the weakest rubric criterion only—ideas, organisation, language or accuracy—then students revise a short paragraph.",outside:"Assign a focused writing worksheet/task in the LMS plus an extra paper worksheet targeting the weak rubric criterion. Students revise and submit before/after work. Teacher provides additional support to priority students whenever suitable during the school day.",measure:"Use the same writing rubric criterion each review and record the criterion score, not just a total mark.",target:"Improve the identified rubric criterion by at least one performance level / agreed rubric step and sustain it in the next writing task.",extension:"Independent extended response with stronger cohesion, vocabulary choice and editing."},
                "Listening": {focus:"key-word listening, detail and comprehension",classroom:"Pre-teach only essential words, set a listening purpose, play once for gist and again for detail, then students justify answers from notes/key words.",friday:"During Vocabulary Bridge Detectives, use a short audio with the priority group: first listen for gist, second for key details, then a 3–5 item comprehension check.",outside:"Assign a short listening worksheet/activity in the LMS plus an extra paper listening-support worksheet for notes and comprehension. Teacher provides additional support to priority students whenever suitable during the school day.",measure:"Listening comprehension accuracy plus number of key details correctly captured.",target:"Reach ≥70% on two comparable listening checks.",extension:"Longer audio, inference, note-taking and oral/written summary."},
                "Speaking": {focus:"fluency, accuracy, vocabulary and response development",classroom:"Use structured sentence stems first, rehearsal with a partner, then an individual response. Give feedback on one speaking criterion at a time.",friday:"During Vocabulary Bridge Detectives, hold a short speaking circle with the priority group using one prompt; rehearse, respond individually and receive one immediate improvement point.",outside:"Assign a short speaking prompt/checklist through the LMS plus an extra paper speaking practice sheet with sentence stems or success criteria. Teacher provides additional support/rehearsal to priority students whenever suitable during the school day.",measure:"Use the same short speaking checklist/rubric: response completion, clarity, target vocabulary and accuracy.",target:"Improve the identified speaking criterion by one rubric step and demonstrate it in two speaking checks.",extension:"Longer unrehearsed response, justification and follow-up questions."},
                "Context Clues": {focus:"using surrounding text to infer unfamiliar word meaning",classroom:"Model how to read before/after the unknown word, identify clue types, substitute a possible meaning and reread to check whether it makes sense.",friday:"During Vocabulary Bridge Detectives, teach the priority group 4–5 short context-clue examples linked to the weekly vocabulary, moving from teacher modelling to independent inference.",outside:"Assign an online context-clues worksheet in the LMS plus an extra paper worksheet with short contextual examples. Teacher provides additional support to priority students whenever suitable during the school day.",measure:"5-item context-clue check: inferred meaning + correct clue identified.",target:"Achieve at least 4/5 correct on two consecutive context-clue checks.",extension:"Infer meaning in longer unfamiliar texts and explain which textual clue supports the inference."},
                "General English": {focus:"the specific errors shown in the selected assessment",classroom:"Sort errors into the two most common needs. Reteach the highest-priority need in a short teacher group, model it, practise together, then check independently.",friday:"While the class completes Vocabulary Bridge Detectives, use the Friday teacher table for the priority students and reteach the most common error from this assessment.",outside:"Assign a targeted online worksheet in the LMS plus an extra paper worksheet based on the student’s actual errors. Teacher provides additional support to priority students whenever suitable during the school day.",measure:"Use a short check matched to the selected assessment and compare the same skill over time.",target:"Move below-expectation students to ≥50%, then toward ≥70%, using comparable evidence.",extension:"Apply the assessed skill independently in a more challenging task."}
              };
              const sourceStrategies: Record<string,string[]> = {
                "Vocabulary":["Visual aids and realia","Multisensory learning","Build vocabulary through engaging contextual practice","Technology and digital tools","Peer learning and collaboration"],
                "Reading":["Visual aids","Differentiate instruction","Simplify language and instructions","Regular feedback","Peer learning and collaboration"],
                "Grammar":["Multisensory learning","Differentiate instruction","Regular feedback","Technology and digital tools","Simplify instructions"],
                "Spelling":["Multisensory learning","Regular feedback","Differentiate instruction","Technology and digital tools","Vocabulary building"],
                "Writing":["Differentiate instruction","Regular feedback","Simplify instructions into manageable steps","Peer learning and collaboration","Technology and digital tools"],
                "Listening":["Multisensory learning","Visual support","Differentiate instruction","Simplify instructions","Regular feedback"],
                "Speaking":["Positive and inclusive learning environment","Peer learning and collaboration","Regular feedback and encouragement","Differentiate instruction","Vocabulary building"],
                "Context Clues":["Visual aids","Vocabulary building","Differentiate instruction","Simplify instructions","Peer learning"],
                "General English":["Differentiate instruction based on individual needs","Regular feedback and encouragement","Visual/multisensory support","Technology and digital tools","Peer learning and collaboration"]
              };
              const recommendedStrategies = sourceStrategies[skill] || sourceStrategies["General English"];
              const p = plans[skill] || plans["General English"];
              return '<section class="card"><h2>4. Recommended Teaching Strategies</h2><p><b>Selected for this need:</b> ' + recommendedStrategies.map(x => safe(x)).join(' · ') + '</p><p>These strategies are adapted to the identified skill need from BridgeUniverse guidance for supporting struggling English language learners; they complement the UAE School Inspection Framework-based analysis.</p></section><section class="card"><h2>5. Student Groups Requiring Action</h2><p><b>Plan generated for:</b> ' + safe(skill) + ' — ' + safe(p.focus) + '.</p><table><thead><tr><th>Group</th><th>Students</th><th>Specific response</th><th>Measurement</th></tr></thead><tbody>' +
                '<tr><td><span class="pill r">Below</span></td><td>' + names(priority) + '</td><td>' + safe(p.classroom) + '</td><td>' + safe(p.measure) + '</td></tr>' +
                '<tr><td><span class="pill y">In line</span></td><td>' + names(inlineRows) + '</td><td>Consolidate ' + safe(p.focus) + ' and add one challenge/application step before independent work.</td><td>Use the same ' + safe(skill) + ' measure and look for movement toward ≥70%.</td></tr>' +
                '<tr><td><span class="pill g">Above</span></td><td>' + names(aboveRows) + '</td><td>' + safe(p.extension) + '</td><td>Maintain ≥70% while demonstrating successful higher-level application.</td></tr></tbody></table></section>' +
                '<section class="card"><h2>6. SMART Improvement Target</h2><p class="analysis"><b>Class target:</b> ' + targetText + '</p><p><b>' + safe(skill) + ' intervention target:</b> ' + safe(p.target) + '</p></section>' +
                '<section class="card"><h2>7. Teaching & Intervention Action Plan — ' + safe(skill) + '</h2><table><thead><tr><th>When</th><th>Action</th><th>Who</th><th>Measure</th><th>Decision</th></tr></thead><tbody>' +
                '<tr><td>Normal lessons</td><td>' + safe(p.classroom) + '</td><td>' + belowRows.length + ' priority students</td><td>' + safe(p.measure) + '</td><td>Continue until the skill target is secure on two checks.</td></tr>' +
                '<tr><td>Every Friday</td><td><b>Vocabulary Bridge Detectives + teacher intervention table:</b> ' + safe(p.friday) + '</td><td>Whole class project + identified weak students with teacher</td><td>Baseline → Friday Check 1 → Check 2 → Check 3 → review assessment.</td><td>If improving, continue/graduate support. If not, narrow the gap and intensify reteaching.</td></tr>' +
                '<tr><td>Outside class</td><td>' + safe(p.outside) + '</td><td>Students needing additional practice</td><td>' + safe(p.measure) + '</td><td>Use errors to plan the next short intervention.</td></tr></tbody></table></section>' +
                '<section class="card"><h2>8. How Progress Will Be Measured</h2><p><b>Do not use one generic measure for every skill.</b> For this analysis use: <b>' + safe(p.measure) + '</b></p><p>Record <b>Baseline → Check 1 → Check 2 → Check 3 → Review Assessment</b>. Keep the task/criterion comparable. The teacher can then show whether each priority student improved, maintained or needs further intervention.</p><p><b>Success target:</b> ' + safe(p.target) + '</p></section>';
            })() +            '<section class="card"><h2>9. Monitoring & Next Assessment</h2><p>Use ongoing formative assessment to check response to teaching. Reassess using a <b>comparable curriculum-aligned evidence point</b>. When the later result is available, use <b>Progress Analysis</b> to compare matched students against their starting points and review whether the intervention should continue, intensify or move to greater challenge.</p></section>' +
            '<section class="card"><h2>10. Student Attainment Profile</h2><table><thead><tr><th>Student ID</th><th>Student name</th><th>Result</th><th>Level</th><th>Next step</th></tr></thead><tbody>' + rows.map(x => { const lev = x.value >= 70 ? ["Above expectations","g","Extend and challenge"] : x.value >= 50 ? ["In line with expectations","y","Consolidate and move toward above"] : ["Below expectations","r","Targeted intervention and follow-up"]; return '<tr><td>' + safe(x.studentId) + '</td><td><b>' + safe(x.name) + '</b></td><td>' + x.value + '%</td><td><span class="pill ' + lev[1] + '">' + lev[0] + '</span></td><td>' + lev[2] + '</td></tr>'; }).join("") + '</tbody></table></section>';
        };
