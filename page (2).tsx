...
export default function Home({
  ...
  const [classes, setClasses] = useState<ClassRoom[]>([]),
    ...
    [gradeFilter, setGradeFilter] = useState<string>(""),
  ...
  useEffect(() => {
    ...
    return onSnapshot(classQuery, (snapshot) => {
      ...
      setSelectedId((v) => {
        if (admin) return classRows.some((c) => c.id === v) ? v : "";
        return classRows.some((c) => c.id === v) ? v : (classRows[0]?.id ?? "");
      });
  ...
  useEffect(() => {
    ...
    if (actingAsAdmin && role === "admin") {
      ...
      const gradeClasses = classes.filter((c) => classGradeLevel(c) === gradeFilter);
      ...
      const testGroups = new Map<string, Test[]>();
      const publishStudents = () => setStudents(Array.from(studentGroups.values()).flat());
      const publishTests = () => setTests(Array.from(testGroups.values()).flat());
      gradeClasses.forEach((classroom) => {
        const scoped = (name: string) =>
          query(collection(db, name), where("classId", "==", classroom.id));
        stops.push(
          onSnapshot(scoped("students"), (snapshot) => {
            studentGroups.set(
              classroom.id,
          ...
          onSnapshot(scoped("assessments"), (snapshot) => {
            testGroups.set(
              classroom.id,
              snapshot.docs.map((d) => ({ id: d.id, ...d.data() }) as Test),
            );
            publishTests();
          }),
        );
    ...
    stops.push(
      onSnapshot(classSource("students"), (s) =>
        setStudents(s.docs.map((d) => ({ id: d.id, ...d.data() }) as Student)),
      ),
      onSnapshot(classSource("assessments"), (s) =>
    ...
    return () => stops.forEach((stop) => stop());
  }, [user, actingAsAdmin, selectedId, role, gradeFilter, classes]);
  ...
  const selectedGradeClasses = useMemo(
    () =>
      gradeFilter
        ? classes.filter((classroom) => classGradeLevel(classroom) === gradeFilter)
        : [],
    [classes, gradeFilter],
  );
  const summary = useMemo(() => {
    const completed = selectedGradeClasses.reduce(
        (n, c) =>
          n +
          planTypes.reduce(
            (x, t) =>
      ...
      required = selectedGradeClasses.length * totalRequired;
    return {
      completed,
      required,
      rate: required ? Math.round((completed / required) * 100) : 0,
    };
  }, [selectedGradeClasses, tests, totalRequired]);
  const selectedGradeActionCount = selectedGradeClasses.filter((c) =>
    planTypes.some(
      (t) =>
        tests.filter((x) => x.classId === c.id && x.type === t).length < plan[t],
    ),
  ).length;
  ...
  return (
    <main className="app-shell">
      ...
      {dataLoading ? (
        ...
        <>
          <section className="panel grade-on-demand">
            <div className="panel-head">
              <div>
                <h2>Choose a grade</h2>
                <p>Only the grade you select will load. Open a class to load its detailed marks and comments.</p>
              </div>
              {gradeFilter && (
                <button className="secondary" onClick={() => setGradeFilter("")}>
                  Change grade
                </button>
              )}
            </div>
            {!gradeFilter && (
              <div className="grade-choice-grid">
                {gradeLevels.map((grade) => {
                  const activeCount = classes.filter((c) => classGradeLevel(c) === grade).length;
                  return (
                    <button
                      key={grade}
                      className="grade-choice"
                      onClick={() => setGradeFilter(grade)}
                    >
                      <span>{grade.includes("ADV") ? "Advanced" : grade.endsWith(" G") ? "General" : "Grade"}</span>
                      <strong>{grade}</strong>
                      <small>{activeCount} active {activeCount === 1 ? "class" : "classes"}</small>
                    </button>
                  );
                })}
              </div>
            ...
            {gradeFilter && (
              <div className="selected-grade-banner">
                <span>Selected grade</span>
                <strong>{gradeFilter}</strong>
                <small>{selectedGradeClasses.length} active {selectedGradeClasses.length === 1 ? "class" : "classes"} loaded</small>
              </div>
            )}
          </section>
          {gradeFilter ? (
          ...
          <section className="stats">
            <article>
              <span>{gradeFilter} completion</span>
            ...
            <article>
              ...
              <strong>{selectedGradeClasses.length}</strong>
              <small>In the selected grade</small>
            </article>
            <article>
          ...
          <section className="panel">
            <div className="panel-head">
              <div>
                <h2>{gradeFilter} class follow-up</h2>
                <p>
            ...
            <div className="table-wrap">
              <table>
                ...
                <tbody>
                  {!selectedGradeClasses.length && (
                    <tr>
                      <td colSpan={9} className="empty">
                          No active classes are available for this grade yet.
                      </td>
                    </tr>
                  )}
                  {selectedGradeClasses.map((c) => {
            ...
            <section className="panel grade-waiting">
              <h2>Select a grade to begin</h2>
              <p>No student, assessment, mark, or comment data is loaded until you choose a grade.</p>
            </section>
          )}
        </>
      ) : (
